...
class ProductRepository {
  ...
  async createProduct({ name, price, description, imgName, categoryId, organizationId }) {
    try {
      const result = await database.executeQuery({
        query: "INSERT INTO product (name, price, description, img_name, fk_category_id, fk_organization_id) VALUES ($1, $2, $3, $4, $5, $6) RETURNING *",
  ...
  async updateProduct({ id, name, price, description, imgName, categoryId }){
    try{
      const result = await database.executeQuery({
        query: `UPDATE product SET name = $2, price = $3, description = $4, img_name = $5, fk_category_id = $6 WHERE id = $1 RETURNING *`,
